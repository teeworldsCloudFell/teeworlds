...
void CLayerTiles::Render()
{
	...
	if(m_Switch)
		m_pEditor->RenderTools()->RenderSwitchmap(((CLayerSwitch*)this)->m_pSwitchTile, m_Width, m_Height, 32.0f, vec4(1,1,1,1), LAYERRENDERFLAG_OPAQUE|LAYERRENDERFLAG_TRANSPARENT);
}
...
void CLayerTiles::BrushFlipX()
{
	...
	if(!m_Game && !m_Tele && !m_Speedup && !m_Switch)
	for(int y = 0; y < m_Height; y++)
		for(int x = 0; x < m_Width; x++)
...
int CLayerTiles::RenderProperties(CUIRect *pToolBox)
{
	...
	if(m_pEditor->m_Map.m_pGameLayer != this && m_pEditor->m_Map.m_pTeleLayer != this && m_pEditor->m_Map.m_pSpeedupLayer != this && m_pEditor->m_Map.m_pSwitchLayer != this)
	{
	...
	if(m_pEditor->m_Map.m_pGameLayer == this || m_pEditor->m_Map.m_pTeleLayer == this || m_pEditor->m_Map.m_pSpeedupLayer == this || m_pEditor->m_Map.m_pSwitchLayer == this) // remove the image and color properties if this is the game-, tele- or speedup layer
	{
		aProps[3].m_pName = 0;
		aProps[4].m_pName = 0;
	}
